...
bool pick_nearest_vertices(std::vector<unsigned int> &verts, Eigen::Ref<const Eigen::Vector3d> win, 
    ...
    Eigen::Vector3f win_0(win(0), win(1), win(2));//近平面
    Eigen::Vector3f win_1(win(0), win(1), 1.);//远平面
    ...
    const auto & shoot_ray = [&V,&F](const Eigen::Vector3f& s, const Eigen::Vector3f& dir, igl::Hit & hit)->bool//发射光线
    {
